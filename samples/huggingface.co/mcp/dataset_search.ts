...
export const dataset_searchToolWithClient = (getClient: () => Promise<Client> | Client) => tool({
  ...
  inputSchema: z.object({ "query": z.string().describe("Search term. Leave blank and specify \"sort\" and \"limit\" to get e.g. \"Top 20 trending datasets\", \"Top 10 most recent datasets\" etc\" ").optional(), "author": z.string().describe("Organization or user who created the dataset (e.g., 'google', 'facebook', 'allenai')").optional(), "tags": z.array(z.string()).describe("Tags to filter datasets (e.g., ['language:en', 'size_categories:1M<n<10M', 'task_categories:text-classification'])").optional(), "sort": z.enum(["trendingScore","downloads","likes","createdAt","lastModified"]).describe("Sort order: trendingScore, downloads, likes, createdAt, lastModified").optional(), "limit": z.number().gte(1).lte(100).describe("Maximum number of results to return").default(20) }).strict(),
